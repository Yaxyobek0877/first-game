...
var _cur_ammo: int = 0           ## O'q-dori yorlig'i uchun (ammo + zaxira magazin birga)
var _max_ammo: int = 0
var _reserve_mags: int = 0
...
func _ready() -> void:
	...
	Events.mags_changed.connect(_on_mags_changed)
...
## Zaxira magazinlar o'zgarganda (reload bittasini kamaytiradi).
func _on_mags_changed(reserve: int) -> void:
	_reserve_mags = reserve
	_refresh_ammo()


## O'q-dori: faqat raqamlar (ikon yonida) — so'zsiz. Magazin soni alohida (mag ikoni yonida).
func _refresh_ammo() -> void:
	ammo_label.text = "%d / %d" % [_cur_ammo, _max_ammo]
	mag_label.text = "×%d" % _reserve_mags
...
func _on_weapon_changed(weapon_name: String) -> void:
	# So'z yo'q — faqat ikon (qurol turiga mos: snayper / topponcha / avtomat).
